...
from .services import collection_name, look_book_collection
...
def save_clothing_item_to_firestore(data, uid):
    doc_ref = db.collection("users").document(uid).collection(collection_name).document(str(data["id"]))
    doc_ref.set(data)

def get_clothing_items_from_firestore(uid):
    items_ref = db.collection("users").document(uid).collection(collection_name)
...
def get_item_data_by_vector_id(uid, vector_id):
    ...
    collection = db.collection("users").document(uid).collection(collection_name)
